...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun  6 02:02:36 2020
...
class Algoritmo:
    ...
    def medir_imagen(self,medida_ref,cedula):
        ...
        canny = cv2.Canny(img_limpia,0, 255)                                            #canny = es un algoritmo detección de bordes que utiliza varias etapas para detectar una amplia gama de bordes en las imágenes (img,umbral min,unbral max) 
        ...
        #        
            ...
            cv2.imwrite("../Imagenes_Huellas/Muestras_procesada/%d.png"%cedula,img_limpia)
        ...
        if conta>3:
            ...
            for cnt in cnts:
                ...
                #print("Angulo=",angulo)
                box = cv2.boxPoints(box)                                                    #calcula los puntos de las esquinas -- para dibujar este rectángulo, necesitamos 4 esquinas del rectángulo.
                box = np.array(box, dtype="int")                                            #transforma todo  a entero
                box = perspective.order_points(box)                                         #ordena los puntos en sentido de las manecillas del reloj
                (tl, tr, br, bl) = box
                contornos.append(box)
            ...
            img_linea = np.array(cv2.line(img_limpia,(cx1,cy1),(cx2,cy2),(100,100,0),1)) 
            
            cx3,cy3=centroides[3]
            cx4,cy4=centroides[4]
            img_linea = np.array(cv2.line(img_limpia,(cx3,cy3),(cx4,cy4),(100,100,0),1))    #fin linea entre centroides
                       
            #cv2.imshow('img2',img_limpia)
            cv2.imwrite("rectangulos_linea_centroide.png",img_limpia )                      #se guarda la imagen con la linea entre centroides
            ...
            for cnt in cnts2:
                ...
                esquinas.append(box.astype("int"))
                print("tl, tr, br, bl=",tl, tr, br, bl)
                cv2.drawContours(img_linea_amarilla, [box.astype("int")], -1, (0, 0, 255), 1)
            ...
            x1,y1=box1[1]
            x2,y2=box1[2]
            img_linea = np.array(cv2.line(img_linea_amarilla,(x1,y1),(x2,y2),(0,255,0),1))  
            xx1,yy1=box2[0]
            xx2,yy2=box2[3]
            img_linea = np.array(cv2.line(img_linea_amarilla,(xx2,yy2),(xx1,yy1),(0,255,255),1)) 
